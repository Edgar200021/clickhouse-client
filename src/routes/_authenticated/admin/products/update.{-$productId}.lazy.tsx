...
import { UpdateProductForm } from "@/components/Admin/forms/product/UpdateProductForm";
...
function RouteComponent() {
	...
	const { productAdmin, isLoading } = useGetProduct(Number(productId));
	...
	return (
		<div className="flex flex-col gap-y-20">
			<h1 className="text-4xl font-bold">Обновление Продукта</h1>
			<AdminManufacturerList hidden />
			<UpdateProductForm
				product={productAdmin}
				onSuccess={() => toast.success("Продукт успешно обновлен")}
			/>
		</div>
	);
}
